...
import { SiBaserow, SiGithub, SiTimescale } from "react-icons/si";
    ...
    return(
        <div className={`text-white fixed w-full top-0 flex items-center z-50 ${moved ? 'bg-gray-800' :''}`}>
            <div className="flex items-center md:pl-10 py-3 pl-3">
                <span className="text-[White] cursor-pointer ml-2 font-bold text-3xl"><SiGithub/></span>
            </div>
            <div className="flex-grow"></div>
            <button className="text-xl lg:hidden border rounded mr-4 p-1 overflow-hidden cursor-pointer" onClick={toggleMenu}>{menuOpen ? <SiTimescale/> : <SiBaserow/>}</button>
